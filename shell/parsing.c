...
expand_environ_var(char *arg)
{
	if (arg[0] == '$') {
		...
		if (arg[1] == '?') {  // Case for pseudo-variable "$?"
			char status_str[12];
			sprintf(status_str,
			        "%d",
			        status);  // Cast int status to string
			environ_var_value = status_str;
		} else {  // Case for all other environment variables
			environ_var_value = getenv(
			        arg +
			        1);  // +1 to send char* without first char '$'
			if (!environ_var_value) {
				return strcpy(arg, "");
			}
		}
		size_t len_environ_var_value = strlen(environ_var_value);
		if (strlen(arg) < len_environ_var_value) {
			arg = realloc(arg, sizeof(char) * len_environ_var_value);
		}
		strcpy(arg, environ_var_value);
	}
	return arg;
}
